...
- **Quattro decisioni chiuse dopo la revisione** (26/07, vedi [decisioni.md](decisioni.md)): **il potere cresce con le settimane** (il ferro diventa noccioline, la sfida sta al tier attuale, il freno è la curva dei prezzi — quindi jetbooster e overclock sono *desiderati*, non buchi da tappare) · **consumabili stackabili e in pausa a gioco chiuso** (durata in tempo di gioco attivo, salvata come secondi rimanenti) · **spedizioni della flotta pagate in pezzi/materiali/oggetti**, niente Cookie per ora · **focus della torretta = priorità con ripiego** · **grammatica dei moduli con schermata di funzione** estesa anche alla sala comunicazioni

- **Rifiniture 43-48 implementate** (26/07, entreranno nella v0.12; compilazione batch pulita e prova di fumo superata):
  - **Costi `disponibili/necessari`** (idea 48): `SetCost` inverte l'ordine — con 30 lingotti su 60 si legge `30/60`. Colori e cifre compattate invariati; gli altri punti di spesa (magazzino, missioni, barra risorse, hangar) erano già in quest'ordine
  - **Zoom vero sulla mappa** (idea 43): `MapPanZoom` ancora lo zoom al **punto tra le dita** — quel punto della mappa resta fermo mentre si pizzica. Prima si scalava attorno al centro dell'area virtuale e la mappa scappava in alto/basso da sola; il calcolo tiene conto dell'ancora di Target (in alto al centro, non al centro)
  - **La fonderia apre il forno** (idea 44): il tap sulla riga entra nella funzione; dentro, pulsante **POTENZIAMENTI** che diventa **"(MAX)"** e spento a fonderia Lv 2. Stessa regola sulla **sala comunicazioni**: il tap apre le missioni, i potenziamenti stanno dietro il pulsante in fondo (pannello "RETE DI BORDO" separato)
  - **"COSTRUISCI MODULO" nascosto** (idea 45) quando non c'è nulla da costruire; la condizione si ricalcola ogni frame, quindi il tasto **torna** appena Argo sblocca sala mappe e sala comunicazioni (e chiude il popup se era aperto)
  - **Focus dell'IA** (idea 46): quarta card nel pannello del laser (compare con l'IA installata e il rame in gioco), pulsante **CAMBIA** che cicla AUTOMATICO → FERRO → RAME. È una **priorità**: `PickAutoTarget` cerca prima il tipo scelto e ripiega sul più basso agganciabile. Il tap resta sopra a tutto e il gating vale sempre
...
- **7 voci nuove dal Google Doc** (26/07, dopo la v0.12 — idee **49-55** in [idee.md](idee.md)): **levetta della velocità** nel mining, **pan+zoom insieme** sulla mappa, **un mercante solo** libero e più veloce, **cambio destinazione in rotta**, **sistema solare con orbite ellittiche**, **X che risale di un livello** (via "◀ SISTEMI") e **nome + X dentro un banner**. Tre punti aperti in [decisioni.md](decisioni.md): la causa vera dello "spalmare danno" (l'IA riscegle il bersaglio a ogni laserata: va impegnata sul suo), il rubinetto secondario dei Cookie con un mercante solo, e il minimo di durata delle rotte interne quando i campi orbitano
...
>
> Poi si riparte dalla roadmap qui sotto: il **26/07** sono arrivate 15 voci nuove dal Google Doc in due tornate. Le prime sei (idee 43-48) sono già dentro la **v0.12**, pubblicata; le sette della seconda tornata (idee **49-55**) sono tutte su **mappa e viaggio** e formano il blocco successivo — prima dei **minerali nuovi**, che restano il prossimo blocco di *contenuto*. Tre punti vanno discussi prima di toccare il codice: sono in cima alle decisioni aperte in [decisioni.md](decisioni.md).

**Prossimo passo (roadmap aggiornata il 26/07 — dettagli nel [GDD](GDD.md)):**
1. ~~**Rifiniture di interfaccia e comandi**~~ ✓ (26/07, idee 43-48, **pubblicate nella v0.12**). Da guardare al playtest sul telefono: i costi nella nuova lettura, il pinch sulla mappa, il giro fonderia → POTENZIAMENTI → indietro, e il focus dell'IA in un campo misto
2. **Mappa e viaggio, seconda passata (idee 49-55)** — nell'ordine: **banner del titolo + X che risale di un livello** (54-55) e **pan+zoom insieme** (50), che sono piccoli; poi il **sistema solare con le orbite** (53) e il **cambio destinazione in rotta** (52), che sono strutturali; **un mercante solo** (51) e la **levetta della velocità** con l'IA che si impegna sul bersaglio (49). Prima di partire: i tre punti aperti in [decisioni.md](decisioni.md)
3. **Silicio, titanio, alluminio** coi Mk successivi (pesi crescenti), nuovi campi e nuove ricette — il ciclo "a mano prima, automatico poi" è già pronto ad accoglierli, e la fonderia a livelli lo rende meccanico: un livello per minerale
4. **Altre stazioni** come checkpoint con gate di teletrasporto (nel grafo: **archi speciali**) e milestone successive; **intro a fumetto**; l'**universo 2** come capitolo massimo
5. **Milestone "Relitti"**: relitti di stazione esplorabili con minigioco, HP della tuta ed equipaggiamento — dopo tutto il resto. Porta con sé l'**inventario/stiva**, che serve anche ai **consumabili** (idea 42: jetbooster, overclock, adrenalina, health pack) e ai **pezzi** delle navi
6. **Milestone "Flotte"** (idea 41, **dopo i relitti**: i pezzi delle navi vengono da lì): costruzione di astronavi, **spedizioni autonome** gestite dalla sala comunicazioni, più avanti i **pirati stellari** — da progettare risolvendo prima la questione del reddito passivo
7. Da rivedere dopo il playtest della v0.12: taratura della banda 0,75-1,00 Cookie/eq se i tempi di viaggio sul grafo si rivelano diversi; eventuale **modulo sensori** per vedere le rotte dei mercanti da più lontano (oggi si vedono tutte dopo il primo incontro); tetto agli asteroidi grossi (idea 27, rinviata)
