...
- **Asset dello store + colonna sonora** (23/07, notte):
  ...
  - **Laserata = prefab `VFX_Flowing_Beam_Main`** del pacchetto "Full Opaque Spell" (copiato in `Resources/VFX/`): un'istanza per laserata orientata sul bersaglio (che segue mentre scende); la **vita delle particelle del getto è tarata su distanza/velocità** così il flusso muore esattamente sull'asteroide, il sotto-oggetto "Impact" è inchiodato sul bersaglio, e a fine laserata gli emitter si fermano (il prefab si autodistrugge da solo — è il suo `MainSpellManager`). Essendo materiali opachi il VFX vive a z negativa (resta sopra le sprite via depth). Fallback sul fascio a due strati disegnato a mano se il prefab manca. Gli emitter "Drop_Out" e "Particle_Hang" del prefab (gocce con gravità e pulviscolo all'indietro, pensati per un raggio orizzontale) vengono spenti allo spawn: col fascio in verticale spruzzavano sotto la nave, come una mira sbagliata. Il getto termina **esattamente sul centro dell'asteroide** grazie a un piano di collisione che segue il bersaglio (perdita di vita totale al contatto): prima la fine del fascio vagava lungo la roccia al variare di distanza e deriva
